...
from flask import render_template, request, redirect, url_for, flash, send_file, send_from_directory ,session, copy_current_request_context
...
def downloadtranscript():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            # iterating through each element of list srt and writing into to buffer
            buffer = BytesIO()
            for i in srt:
                buffer.write(f'{i}'.encode('utf-8'))
                buffer.write(b'\n')
            ...
            return send_file(
                buffer,
                download_name=url.title +' - Transcript.txt',
                as_attachment=True, 
                mimetype='text/csv'
            )
...
def bulkdownloadvideo():  
    ...
    if request.method == 'POST':
        ...
        # Saving the file uploaded by user in /tmp folder in GCP.
